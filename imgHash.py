...
ROOT_DIR = os.getcwd()
from_path = "/static/datasets/animal-10/raw-img/"
# 线程中访问全局变量声明
global result
global temp
global image1
...
def Run_threadpool(image_to_search, label, number=4):
    """
    多线程图片相似度计算入口
    :param image_to_search: 已上传的图片
    :param label: 上传图片标签
    :param number: 返回图片数量
    :return: 返回的图片路径与相似度
    """
    global image1, image_label, temp, result
    # 变量处理
    image1 = cv2.imread(image_to_search)
    image_label = label
    temp = []
    result = []
    listx = [i for i in os.listdir(os.getcwd() + "/datasets/animal-10/raw-img/" + image_label)]

    # 开启线程池
    pool = threadpool.ThreadPool(16)
    request = threadpool.makeRequests(Compute_Hash, listx)
...
def Compute_Hash(image):
    global temp, result
    image2 = cv2.imread(os.getcwd() + "/datasets/animal-10/raw-img/" + image_label + "/" + image)
    d_dist = Hamming_distance(dHash(image1), dHash(image2))
    trust_rate = (1 - d_dist * 1.0 / 64)

    temp.append(trust_rate)
    result.append({
        "file_name": f"{from_path}{image_label}/{image}",
        "score": trust_rate
    })


if __name__ == '__main__':
    from_path = "../datasets/animal-10-jpg/train/"
    name = "mucca"
    image1 = cv2.imread("OIP-_2iBsOsobKZsP76-9Cd-qAHaEM.jpg")
    result = {"score": 0, "file_name": ""}

    start1 = time.time()
    for file_name in [listx for listx in os.listdir(from_path + name)]:
        image2 = cv2.imread(filename=from_path + name + "/" + file_name)
        d_dist = Hamming_distance(dHash(image1), dHash(image2))
        if result["score"] < (1 - d_dist * 1.0 / 64):
            result["file_name"] = file_name
            result["score"] = (1 - d_dist * 1.0 / 64)
    end1 = time.time()

    print('file name is ' + result["file_name"] + ', similarity is ' + '%f' % result["score"] + ', time is ' + '%f' % (
            end1 - start1))
